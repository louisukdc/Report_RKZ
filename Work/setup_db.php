...
echo "<h3>Menjalankan Setup Database Otomatis...</h3>";
...
try {
    ...
    $conn->exec($sql);
    echo "<p>✔️ Tabel berhasil dibuat / dipastikan ada.</p>";
    ...
    // 3. Insert Kwitansi Header (Dummy Data)
    $conn->exec("INSERT IGNORE INTO `Kwit_manual_h` (`st`, `no_kwitansi`, `no_faktur`, `terima_dari`, `Jumlah`, `keterangan1`, `keterangan2`, `user`, `tgl_kwitansi`, `jam`) VALUES
    ...
    echo "<p>✔️ Data Dummy Kwitansi berhasil dimasukkan ke database.</p>";

    // 4. Insert Detail & Master
    $conn->exec("INSERT IGNORE INTO `Kwit_manual_d` (`id`, `no_faktur`, `Kd_brg`, `nama`, `jumlah`) VALUES
    ...
    $conn->exec("INSERT IGNORE INTO `master` (`kode`, `Nama`) VALUES
    ...
    $conn->exec("INSERT IGNORE INTO `master_nomor` (`No_Kwt`, `No_Fkt`) VALUES ('26041', '121')");
    
    echo "<p>✔️ Seluruh Setup selesai!</p>";
    echo "<a href='laporan.php' style='display:inline-block; padding: 10px 20px; background:#2563eb; color:#fff; text-decoration:none; border-radius:5px;'>Kembali ke Laporan</a>";
